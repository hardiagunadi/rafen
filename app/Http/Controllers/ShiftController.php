...
class ShiftController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $subUsers = User::where(function ($q) use ($ownerId, $user) {
            $q->where('id', $ownerId)
    ...
    public function dispatchRemindersForOwner(int $ownerId): int
    {
        ...
        foreach ($schedules as $schedule) {
            ...
            try {
                ...
                $service->sendMessage($employee->phone, $msg, ['event' => 'shift_reminder']);
                $sent++;
            } catch (\Throwable) {
                // Continue to next
            }
        ...
        if ($settings->wa_shift_group_number && $schedules->isNotEmpty()) {
            try {
                ...
                $service->sendMessage($settings->wa_shift_group_number, $summary, ['event' => 'shift_reminder_group']);
            } catch (\Throwable) {
                // Non-blocking
            }
        }
