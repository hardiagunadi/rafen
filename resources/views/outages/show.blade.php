    ...
    <div class="col-md-8">
        ...
        <div class="card">
            ...
            <div class="card-body p-0">
                <div class="timeline timeline-inverse p-3" id="updateTimeline">
...
function prependUpdate(u) {
    ...
    const icon = typeIcon[u.type] || 'fas fa-info-circle text-muted';
    const html = `
    <div class="timeline-item">
        <span class="time"><i class="fas fa-clock"></i> ${u.created_at}</span>
        <h3 class="timeline-header ${icon.split(' ').slice(1).join(' ')}">
            <i class="${icon.split(' ')[0]} mr-1"></i>${u.user_name}
            ${!u.is_public ? '<span class="badge badge-light badge-sm ml-1">internal</span>' : ''}
        </h3>
        ${u.meta ? `<div class="timeline-body text-muted small">${u.meta}</div>` : ''}
        ${u.body ? `<div class="timeline-body">${u.body}</div>` : ''}
        ${u.image_url ? `<div class="timeline-body"><a href="${u.image_url}" target="_blank"><img src="${u.image_url}" style="max-height:200px;border-radius:8px;"></a></div>` : ''}
    </div>`;
